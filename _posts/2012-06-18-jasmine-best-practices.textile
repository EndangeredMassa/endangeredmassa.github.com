...
Only certain functions types are async: setTimeout/setInterval, event triggers (except for shortcut calls like .click() in most browsers), and ajax calls. This applies to any methods that depend on this list, which is why it's important to understand the libraries you use. Reading the source, especially <a href="http://backbonejs.org/docs/backbone.html">nicely annotated source</a>, is the best way to do that.
