...
Writing JavaScript is a lot easier when you actually write <a href="http://coffeescript.org/">CoffeeScript</a>. When using CoffeeScript, you don't have to worry about a lot of errors that cause <a href="http://www.jslint.com/">JSLint</a> or <a href="http://www.jshint.com/">JSHint</a> to get angry. It also provides a lot of syntactic sugar that makes it a pleasure to work with. Just promise me you'll take a look at it.

The examples below are written in CoffeeScript and <a href="http://pivotal.github.com/jasmine/">Jasmine</a>. I use explicit return statements to try and make it more clear to those of you not as familiar with CoffeeScript.

h2. Avoiding Global State
...
My definition of a comment does not include what other people consider documentation. If you use comments for code documentation, that's a separate (and probably legitimate) use.

TODO comments like the following should really be logged work items, not comments in the code. If you leave it in the code, it's likely to be forgotten about. And, if someone discovers it later, they will have to find the author of that comment to make sure they do the right thing.

<script src="https://gist.github.com/2950153.js?file=todo_comment.coffee"></script>

Comments about the next few lines of code usually indicate a method in need of extraction. Consider this example.

<script src="https://gist.github.com/2950153.js?file=section_comment.coffee"></script>

In this case, we can turn the comment into the method name. This is called self-documenting code. The best part is that changing the method name will break the code unless you change where it's called. The same is not true for a comment about the code.

<script src="https://gist.github.com/2950153.js?file=section_comment_fixed.coffee"></script>
...
JavaScript runs in a single thread, including the UI. (There's some inconsistencies in browser environments, but this is essentially true.) Blocking that thread blocks the UI. When you create an async callback, that gets saved to the side until the current execution flow ends. Then it will be executed. However, just because you give a method a callback does not mean it's asynchronous.

There is a short list of things that can cause async events:

1. setTimeout/setInterval
2. event handlers (on window, document, DOM fragments, web workers)
3. XMLHttpRequests (ajax)

Note that event handlers can be called directly (without creating an async event) in some cases. The most notable case is that .click() calls on DOM elements will (in all current browsers) call that event's handler directly.

Any library that uses these methods underneath will also create async events. That's why it's important to understand the libraries you use. Reading the source, especially <a href="http://backbonejs.org/docs/backbone.html">nicely annotated source</a>, is the best way to do that.
