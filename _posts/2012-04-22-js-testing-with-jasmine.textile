...
* 1 argument => pending
* 2 arguments => normal behavior
* 3 arguments => requirejs behavior
...
p. Since writing that tool, however, I've changed my view on testing visibility. I'd rather let my CSS handle that and simply test for the existence of a class that should imply visibility. The simply example would be to test an element for a <code>hide</code> class if you want it to be hidden.
